...
class World:
    ...
    def cast_ability(self, hero_id=None, hero=None, ability_name=None, ability=None, cell=None, row=None, column=None):
        ...
        # getting inputs:
        if hero_id is not None:
            selected_hero =self.get_hero(hero_id)
        elif hero is not None:
            selected_hero = hero
        ...
        if ability_name is not None:
            ...
            selected_ability = ability.name

        if cell is not None:
            target_cell = cell
        elif row is not None and column is not None:
            target_cell = self.map.get_cell(row,column)
        selected_ability_constant = self._get_ability_constants(selected_ability)
        # correcting AP:
        self.ap = self.ap - selected_ability_constant.ap_cost
        # correcting turn number:(this line will change it's place in future updates.)
        self.current_turn = self.current_turn+1
        # correcting my_heros :
        # handling dodges:
        if selected_ability_constant.type == AbilityType.DODGE:
            for itr in self.my_heroes:
                if itr.id == selected_hero.id:
                    itr.current_cell = target_cell
            for itr in self.heroes:
                if itr.id == selected_hero.id:
                    itr.current_cell = target_cell

        # handling offensive actions :
        if selected_ability_constant.name == AbilityName.GUARDIAN_FORTIFY:
            for itr in self.my_heroes:
                if itr.current_cell == target_cell:
                    itr.is_shielded = True
            for itr in self.heroes:
                if itr.current_cell == target_cell:
                    itr.is_shielded = True
        if selected_ability_constant.name == AbilityName.HEALER_HEAL:
            for itr in self.my_heroes:
                if itr.current_cell == target_cell:
                    itr.current_hp = np.min(
                        [itr.max_hp, itr.current_hp + selected_ability_constant.power])
            for itr in self.heroes:
                if itr.current_cell == target_cell:
                    itr.current_hp = np.min(
                        [itr.max_hp, itr.current_hp + selected_ability_constant.power])
        ...
        # correcting op_heros:
        kill_counter =0
        if selected_ability_constant.type == AbilityType.OFFENSIVE:
            # creating a list of hero ids that are affected by this action and then searching in it:
            affected_heros = self.get_ability_targets(
                ability=selected_ability, start_cell=selected_hero.current_cell,target_cell=target_cell)
            affected_hero_ids = []
            for itr in affected_heros:
                affected_hero_ids.append(itr.id)
            # applying damages:
            for itr in self.opp_heroes:
                if affected_hero_ids.count(itr.id) > 0 and itr.current_hp > 0:
                    itr.current_hp = np.max(0, itr.current_hp - selected_ability_constant.power)
                    if itr.current_hp == 0:
                        kill_counter = kill_counter + 1
            for itr in self.heroes:
                if affected_hero_ids.count(itr.id) > 0:
                    itr.current_hp = np.max(0, itr.current_hp - selected_ability_constant.power)
        # correcting scores (again this part will probably change its place in future updates.):
        # applying kill counter we have just calculated:
        self.my_score = self.my_score + self.kill_score * kill_counter
        # counting heros in objective zone:
        counter = 0
        for itr in self.my_heroes:
            if itr.current_cell.is_in_objective_zone is True:
                counter  =counter +1
        self.my_score = self.my_score + counter * self.objective_zone_score
        counter = 0
        for itr in self.opp_heroes:
            if itr.current_cell.is_in_objective_zone is True:
                counter = counter + 1
        self.opp_heroes = self.opp_heroes + counter * self.objective_zone_score

        # correcting map:
        my_hero_positions = []
        for itr in self.my_heroes:
            my_hero_positions.append(itr.current_cell)

        for row in range(int(self.map.row_num)):
            for col in range(int(self.map.column_num)):
                vector = np.zeros(4)
                for i in range(0, 3):
                    vector[i] = self.is_in_vision(start_cell=self.map.cells[row][col], end_cell=my_hero_positions[i])
                if np.sum(vector) == 0:
                    self.map.cells[row][col].is_in_vision = False
                else:
                    self.map.cells[row][col].is_in_vision = True
        return
    def move_hero(self, hero_id=None, hero=None, direction=None):
        ...
        # correcting the AP:
        if hero_id is not None:
            hero1 = self.get_hero(hero_id)
        else:
            hero1 = hero

        self.ap = self.ap -hero1.move_ap_cost

        # correcting the move phase number:(this line will change it's place in future updates.)
        self.move_phase_num = self.move_phase_num + 1
        # correcting heros and my_heros lists:
        old_row = hero1.current_cell.row
        old_col = hero1.current_cell.column
        for itr in self.heroes:
            if itr.id == hero1.id:
                if direction == Direction.UP:
                    itr.current_cell = self.map.cells[old_row][old_col-1]
                if direction == Direction.DOWN:
                    itr.current_cell = self.map.cells[old_row][old_col + 1]
                if direction == Direction.LEFT:
                    itr.current_cell = self.map.cells[old_row-1][old_col]
        ...
        for row in range(int(self.map.row_num)):
            for col in range(int(self.map.column_num)):
                vector = np.zeros(4)
                for i in range(0, 3):
                    vector[i] = self.is_in_vision(start_cell=self.map.cells[row][col],
                                                  end_cell=my_hero_positions[i])
                if np.sum(vector) == 0:
                    self.map.cells[row][col].is_in_vision = False
                else:
                    self.map.cells[row][col].is_in_vision = True
